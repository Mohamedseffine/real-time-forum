...
func InsertUser(db *sql.DB, user objects.User, password string)(int64, error ) {
	...
	res , err := stm.Exec(User.Username, User.Name, User.FamilyName, User.Gender, User.Email, password, time.Now())
	if err != nil {
		return -1 , err
	}
